...
import { Client, Batch, QueueItem } from '../types';
import {
  Users,
  Clock,
  Send,
...
export default function DashboardView({
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'Lido':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-indigo-50 text-indigo-600 border border-indigo-100">Lido</span>;
      case 'Entregue':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-emerald-50 text-emerald-600 border border-emerald-100">Entregue</span>;
      case 'Enviado':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-blue-50 text-blue-600 border border-blue-100">Enviado</span>;
      case 'Enviando':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-sky-50 text-sky-600 border border-sky-100 animate-pulse">Enviando</span>;
      case 'Fila':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-amber-50 text-amber-600 border border-amber-100">Em Fila</span>;
      case 'Falhou':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-rose-50 text-rose-600 border border-rose-100">Falhou</span>;
      default:
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-slate-50 text-slate-600 border border-slate-100">{status}</span>;
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        {stats.map((stat, i) => {
          ...
          return (
            <motion.div
              ...
              key={stat.title}
              className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-subtle flex flex-col justify-between"
            >
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
                ...
                <button
                  ...
                  className="inline-flex items-center text-[10px] font-semibold text-blue-600 hover:text-blue-700 gap-0.5"
                >
                  {stat.actionLabel}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Latest Sends */}
        <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-subtle flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4">
            ...
            <div className="space-y-3.5">
              {recentDispatches.length === 0 ? (
                ...
                recentDispatches.map((disp) => (
                  <div key={disp.id} className="flex items-start gap-3 border-b border-slate-100/60 pb-3 last:border-0 last:pb-0">
                    <div className="p-1.5 rounded bg-slate-50 border border-slate-100 text-slate-500 text-[10px] font-semibold">
                      PDF
                    </div>
                    <div className="overflow-hidden flex-1">
                      <p className="text-xs font-semibold text-slate-950 truncate leading-snug">
                        {disp.cliente_nome}
                      </p>
          ...
          <button
            ...
            className="w-full mt-4 py-1.5 bg-slate-50 hover:bg-slate-100/80 border border-slate-200 rounded-lg text-slate-700 text-xs font-semibold transition-all flex items-center justify-center gap-1"
          >
      ...
      <div className="bg-slate-50 rounded-xl border border-slate-200 p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white p-3.5 rounded-lg border border-slate-200 shadow-subtle flex gap-3">
            <div className="w-7 h-7 rounded-full bg-blue-50 text-blue-600 text-xs font-bold flex items-center justify-center shrink-0">1</div>
            <div>
              <h4 className="text-xs font-semibold text-slate-900">Cadastro de Clientes</h4>
              <p className="text-[11px] text-slate-500 mt-0.5">Cadastre ou importe clientes via CSV especificando seu código único (ex: CLI0001).</p>
            </div>
          </div>
          <div className="bg-white p-3.5 rounded-lg border border-slate-200 shadow-subtle flex gap-3">
            <div className="w-7 h-7 rounded-full bg-blue-50 text-blue-600 text-xs font-bold flex items-center justify-center shrink-0">2</div>
            <div>
              <h4 className="text-xs font-semibold text-slate-900">Upload dos Relatórios</h4>
              <p className="text-[11px] text-slate-500 mt-0.5">Arraste múltiplos relatórios em PDF. O sistema extrai o código do cliente pelo nome do arquivo automaticamente.</p>
            </div>
          </div>
          <div className="bg-white p-3.5 rounded-lg border border-slate-200 shadow-subtle flex gap-3">
            <div className="w-7 h-7 rounded-full bg-blue-50 text-blue-600 text-xs font-bold flex items-center justify-center shrink-0">3</div>
            <div>
              <h4 className="text-xs font-semibold text-slate-900">Validação e Despacho</h4>
              <p className="text-[11px] text-slate-500 mt-0.5">Confira na tabela inteligente se há pendências. Com um clique crie o lote para envio imediato ou agendado.</p>
            </div>
          </div>
        </div>
      </div>
